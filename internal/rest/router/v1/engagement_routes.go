...
func (r *Router) registerEngagementRoutes(v1 *mux.Router, engagementHandler *engagement_handler.EngagementHandler, tokenMachine tokenmachine.TokenMachine) {
	users := v1.PathPrefix("/users").Subrouter()
	users.Use(middleware.AuthMiddleware(tokenMachine))
	...
	campaignFunnel.Use(middleware.AuthMiddleware(tokenMachine))
